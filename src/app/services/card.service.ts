...
export class CardService {
	...
	addToCart(product: Product, productAmount: number) {
		const existingItemIndex = this.cardIemList.findIndex(
			(item) => item.id === product.id
		) as number;

		if (existingItemIndex >= 0) {
			this.cardIemList[existingItemIndex].quantity = productAmount;
			this.cardIemList[existingItemIndex].totalPrice =
				this.calculateProductPrice(product.price, productAmount);
		}
		//To deep clone the product object
			...
			this.cardItem.totalPrice = this.calculateProductPrice(
				product.price,
				productAmount
			);
	...
	updateCardList(cardItem: CardItem, amount: number) {
		...
		this.cardIemList[existingItemIndex].totalPrice =
			this.calculateProductPrice(cardItem.product.price, amount);
		return this.cardIemList;
	}
	...
	getTotalCardPrice() {
		return this.cardIemList.reduce((total, cardItem) => {
			...
			total += Number(cardItem.totalPrice);
			return total;
		}, 0);
	}
	calculateProductPrice(productPrice: number, productAmount: number) {
		const total = (productPrice * productAmount).toFixed(
			2
	...
	removeFromCard(cardItem: CardItem) {
		const newList = this.cardIemList.filter(
			(item) => item.id !== cardItem.id
		);
		this.cardIemList = newList;
		console.log(this.cardIemList);
		return this.cardIemList;
	}
}
